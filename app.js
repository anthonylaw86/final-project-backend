...
const spotify_redirect_uri = "http://localhost:3002/auth/callback";
...
app.get("/auth/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(`http://localhost:3000/post-login?token=${data.access_token}`);
  } catch (error) {
    console.error("Error exchanging code for token:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
